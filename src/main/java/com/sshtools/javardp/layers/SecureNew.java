...
public class SecureNew {
	static Logger logger = LoggerFactory.getLogger(SecureNew.class);
	 ...
	 */
	public SecureNew(IContext context, State state, VChannels channels) {
		//
		// UNCOMMENT IF USED!!!!!
		//
//		licence = new Licence(state, context, this);
	 ...
	 */
	public void connect(IO io) throws UnknownHostException, IOException, RdesktopException {
	...
	public byte[] decrypt(byte[] data) throws RdesktopCryptoException {
		try {
			...
			buffer = rc4_dec.doFinal(data);
			this.dec_count++;
			return buffer;
		} catch (InvalidKeyException ike) {
			throw new RdesktopCryptoException("Failed encryption.", ike);
		} catch (IllegalBlockSizeException e) {
			throw new RdesktopCryptoException("Failed encryption.", e);
	...
	public byte[] encrypt(byte[] data) throws RdesktopCryptoException {
		try {
			...
			if (this.enc_count == 4096) {
				...
				this.rc4_enc.init(Cipher.ENCRYPT_MODE, new SecretKeySpec(key, "RC4"));
				// logger.debug("Packet enc_count="+enc_count);
				this.enc_count = 0;
			}
			// this.rc4.engineInitEncrypt(this.rc4_encrypt_key);
			buffer = this.rc4_enc.doFinal(data);
			this.enc_count++;
			return buffer;
		} catch (InvalidKeyException ike) {
			throw new RdesktopCryptoException("Failed encryption.", ike);
	...
	public byte[] encrypt(byte[] data, int length) throws RdesktopCryptoException {
		try {
			...
			// this.rc4.engineInitEncrypt(this.rc4_encrypt_key);
			buffer = this.rc4_enc.doFinal(data, 0, length);
			this.enc_count++;
			return buffer;
		} catch (InvalidKeyException ike) {
			throw new RdesktopCryptoException("Failed encryption.", ike);
	...
	public void generate_keys(int rc4_key_size) throws RdesktopCryptoException {
		...
		System.arraycopy(this.sec_decrypt_key, 0, this.sec_decrypt_update_key, 0, 16); // changed
																						// from
																						// 8
																						// -
																						// rdesktop
																						// 1.2.0
		System.arraycopy(this.sec_encrypt_key, 0, this.sec_encrypt_update_key, 0, 16); // changed
																						// from
																						// 8
																						// -
																						// rdesktop
		...
		try {
			System.arraycopy(this.sec_encrypt_key, 0, key, 0, this.keylength);
			rc4_enc.init(Cipher.ENCRYPT_MODE, new SecretKeySpec(key, "RC4"));
			System.arraycopy(this.sec_decrypt_key, 0, key, 0, this.keylength);
			rc4_dec.init(Cipher.DECRYPT_MODE, new SecretKeySpec(key, "RC4"));
		} catch (InvalidKeyException ike) {
			throw new RdesktopCryptoException("Failed to generate keys.", ike);
		}
	}
	...
	public int parseCryptInfo(Packet data) throws RdesktopException {
		...
		if ((flags & 1) != 0) {
			...
			while (data.getPosition() < data.getEnd()) {
				...
				switch (tag) {
				case (SecureNew.SEC_TAG_PUBKEY):
					if (!parsePublicKey(data)) {
						return 0;
					}
					break;
				case (SecureNew.SEC_TAG_KEYSIG):
	...
	public void processMcsData(Packet mcs_data) throws RdesktopException {
		...
		while (mcs_data.getPosition() < mcs_data.getEnd()) {
			...
			switch (tag) {
			case (SecureNew.SEC_TAG_SRV_INFO):
				processSrvInfo(mcs_data);
				break;
			case (SecureNew.SEC_TAG_SRV_CRYPT):
				this.processCryptInfo(mcs_data);
			case (SecureNew.SEC_TAG_SRV_CHANNELS):
				/*
	 ...
	 */
	public Packet receive() throws RdesktopException, IOException {
	...
	public byte[] update(byte[] key, byte[] update_key) throws RdesktopCryptoException {
		try {
			byte[] shasig = new byte[20];
			byte[] update = new byte[this.keylength]; // changed from 8 -
														// rdesktop
			// 1.2.0
