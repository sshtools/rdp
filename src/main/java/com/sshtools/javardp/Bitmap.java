...
public class Bitmap {
	 ...
	 * @return
	 */
	static int cvalx(Options options, byte[] data, int offset, int Bpp) {
		int rv = 0;
		if (options.server_bpp == 15) {
				...
				case 4: /* Copy */
					while (((count & ~0x7) != 0) && ((x + 8) < width)) {
						for (int i = 0; i < 8; i++) {
							...
							// Bpp);
							w.setRGB(left + x, top + height, cvalx(options, compressed_pixel, input, Bpp));
							input += Bpp;
							count--;
							x++;
						}
				...
				case 4: /* Copy */
					while (((count & ~0x7) != 0) && ((x + 8) < width)) {
						for (int i = 0; i < 8; i++) {
							// pixel[line + x] = cvalx(compressed_pixel, input,
							// Bpp);
							w.setRGB(x, height, cvalx(options, compressed_pixel, input, Bpp));
							input += Bpp;
							count--;
							x++;
						}
	 ...
	 * @throws RdesktopException
	 */
	public static int[] decompressInt(Options options, int width, int height, int size, RdpPacket data, int Bpp)
			throws RdesktopException {
		byte[] compressed_pixel = new byte[size];
		data.copyToByteArray(compressed_pixel, 0, data.getPosition(), size);
		data.incrementPosition(size);
