...
import com.sshtools.javardp.orders.Pen;
...
public class Orders {
	...
	private void processRawBitmapCache(RdpPacket data) throws RdesktopException {
		...
		cache.putBitmap(cache_id, cache_idx, new Bitmap(Bitmap.convertImage(options, inverted, Bpp), width, height, 0, 0), 0);
	}
	...
	private void processBitmapCache(RdpPacket data) throws RdesktopException {
		...
		if (Bpp == 1) {
			...
			if (pixel != null)
				cache.putBitmap(cache_id, cache_idx, new Bitmap(Bitmap.convertImage(options, pixel, Bpp), width, height, 0, 0), 0);
			else
	...
	private void process_bmpcache2(RdpPacket data, int flags, boolean compressed) throws RdesktopException, IOException {
		...
		if (compressed) {
			...
			// *
			// Bpp);
			bitmap = new Bitmap(Bitmap.convertImage(options, bmpdata, Bpp), width, height, 0, 0);
		}
		...
		if (bitmap != null) {
			...
			if ((flags & PERSIST) != 0)
				PstCache.pstcache_put_bitmap(options, cache_id, cache_idx, bitmap_id, width, height, width * height * Bpp, bmpdata);
		} else {
