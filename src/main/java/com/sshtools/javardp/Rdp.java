...
public class Rdp {
	...
	public Rdp(IContext context, State state, VChannels channels) {
		...
		this.state = state;
		secureLayer = new Secure(context, state, channels);
	...
	protected void processBitmapUpdates(Packet data) throws RdesktopException {
		...
		for (int i = 0; i < n_updates; i++) {
			...
			if (compression == 0) {
				...
				surface.displayImage(Bitmap.convertImage(state, pixel, state.getBytesPerPixel()), width, height, left, top, cx, cy);
				continue;
			}
			...
			if (state.getBytesPerPixel() == 1) {
				...
				if (pixel != null)
					surface.displayImage(Bitmap.convertImage(state, pixel, state.getBytesPerPixel()), width, height, left, top, cx,
							cy);
				else
				...
				if (state.getOptions().getBitmapDecompressionStore() == Options.INTEGER_BITMAP_DECOMPRESSION) {
					...
					surface.displayCompressed(left, top, width, height, size, data, state.getBytesPerPixel(), null);
				}
			}
		}
		...
		for (n = 0; n < ncapsets; n++) {
			...
			case RDP_CAPSET_BITMAP:
				if (processBitmapCaps(state, data)) {
					surface.backingStoreResize(state.getWidth(), state.getHeight(), false);
				}
				break;
	...
	private void sendBitmapcache2Caps(Packet data) {
		...
		if (PstCache.pstcache_init(state, 2)) {
	...
	private void sendConfirmActive()
		...
		if (state.isRDP5() && state.getOptions().isPersistentBitmapCaching()) {
	...
	private void sendInputCaps(Packet data) {
		...
		data.setLittleEndian16(INPUT_FLAG_SCANCODES); // flags (SCANCODES
														...
														// crashes if zero)
		data.setLittleEndian16(0); // pad
		data.setLittleEndian32(state.getOptions().getKeylayout());
